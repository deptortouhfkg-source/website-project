...
loginForm.addEventListener("submit", async (e) => {
  ...
  // Buat email tiruan untuk Firebase
  const fakeEmail = username + "@elogbook.com";

  try {
    const userCredential = await firebaseAuth.signInWithEmailAndPassword(window.auth, fakeEmail, password);
    const user = userCredential.user;

    // Ambil data mahasiswa dari Firestore
    const userDoc = await firebaseGetDoc(firebaseDoc(window.db, "mahasiswa", user.uid));
    const userData = userDoc.exists() ? userDoc.data() : {};

    Swal.fire({
      icon: "success",
      title: "Login Berhasil",
      text: `Selamat datang, ${userData.nama || username}!`,
      timer: 2000,
    ...
    $("loginError").innerText = mapFirebaseError(err.code);
  }
});


  
// === ATUR HALAMAN SAAT PERTAMA KALI DIMUAT ===
const loginPage = document.getElementById("loginPage");
const dashboardPage = document.getElementById("dashboardPage");
const registerPage = document.getElementById("registerPage");
const forgotPage = document.getElementById("forgotPage");

// === Navigasi antar form login/register/forgot ===
...
// Pastikan hanya login yang tampil pertama kali
if (loginPage) loginPage.style.display = "flex";
if (dashboardPage) dashboardPage.style.display = "none";
if (registerPage) registerPage.style.display = "none";
if (forgotPage) forgotPage.style.display = "none";

// Jalankan auto-login kalau localStorage masih punya data
...
if (savedRole && savedUser) {
  window.addEventListener("load", () => {
    if (typeof showDashboard === "function") {
      ...
      console.warn("⚠️ Fungsi showDashboard belum siap saat load halaman.");
    }
  });
}
...
if (savedRole && savedUser) {
  ...
  // === Ganti Halaman Form Login/Register/Forgot ===
  const showLogin = () => {
  ...
  const backLogin1 = $("backToLogin1");
  const backLogin2 = $("backToLogin2");

  if (regLink) regLink.addEventListener("click", showRegister);
...
if (registerForm) {
  registerForm.addEventListener("submit", async (e) => {
    ...
    const ovo = $("regOVO").value.trim();
    const emailAsli = $("regEmail").value.trim();
    const password = $("regPassword").value.trim();

    if (!nama || !nim || !ovo || !emailAsli || !password) {
      $("registerError").innerText = "⚠️ Harap isi semua kolom.";
      return;
    }

    // Email tiruan untuk autentikasi login
    const fakeEmail = `${nim}@elogbook.com`;

    try {
      // 🔹 Buat akun Firebase dengan email tiruan
      const userCredential = await firebaseAuth.createUserWithEmailAndPassword(window.auth, fakeEmail, password);
      const user = userCredential.user;

      // 🔹 Simpan data mahasiswa ke Firestore
      await window.firebaseSetDoc(
        window.firebaseDoc(window.db, "mahasiswa", user.uid),
        {
          nama,
          nim,
          ovo,
          emailAsli,  // email asli disimpan untuk reset password
          fakeEmail,  // email login tiruan
      ...
      Swal.fire({
        ...
        text: "Akun berhasil dibuat. Silakan login menggunakan NIM dan password Anda.",
        timer: 2000,
        showConfirmButton: false,
      }).then(() => {
        $("registerPage").style.display = "none";
      ...
      $("registerError").innerText = mapFirebaseError(err.code);
...
// === Tambahan menu PANDUAN ===
const sidebarNav = document.querySelector(".sidebar nav");
if (sidebarNav && !document.querySelector('button[data-tab="panduanContent"]')) {
  ...
  sidebarNav.appendChild(panduanBtn);
