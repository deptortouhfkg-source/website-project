...
loginForm.addEventListener("submit", async (e) => {
  ...
  try {
    const userCredential = await window.firebaseSignIn(window.firebaseAuth, fakeEmail, password);
...
if (savedRole && savedUser) {
  console.log("Terdeteksi login sebelumnya, tapi auto-login dinonaktifkan untuk keamanan.");
  localStorage.clear(); // hapus data otomatis
  if (loginPage) loginPage.style.display = "flex";
  if (dashboardPage) dashboardPage.style.display = "none";
} else {
  if (loginPage) loginPage.style.display = "flex";
